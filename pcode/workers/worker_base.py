...
class WorkerBase(object):
    def __init__(self, conf):
        ...
        dist.barrier()
        self.dataset = create_dataset.define_dataset(conf, data=conf.data, display_log=False)
        ...
        if self.conf.prepare_data == "combine":
            ...
            conf.logger.log(f"Worker-{self.conf.graph.worker_id} initialized the local combined data with master.")

            for client_id in self.client_ids:
                self.local_datasets[client_id] = create_dataset.define_local_dataset(
                    conf, client_id, self.data_partitioner
                )
            conf.logger.log(f"Worker-{self.conf.graph.worker_id} initialized the local dataset split with master.")
        else:
    ...
    def _recv_model_from_master(self):
        ...
        self.conf.logger.log(
            ...
            f"received the global/personal model ({self.arch}) from Master. The model status {'is updated' if old_buffer.norm() != new_buffer.norm() else 'is not updated'}."
        )        
        dist.barrier()        
    ...
    def _send_model_to_master(self):
        dist.barrier()
        self.conf.logger.log(
            f"Worker-{self.conf.graph.worker_id} (client-{self.conf.graph.client_id}) sending the model ({self.arch}) "
            f"back to Master."
        )
