...
def build_mpi_script(conf, replacement=None):
    ...
    if conf.n_participated >= 1:
        prefix_cmd = f"mpirun -n {conf.n_participated + 1} --hostfile {conf.hostfile} " \
                     f"--mca orte_base_help_aggregate 0 --mca btl_tcp_if_exclude docker0,lo --prefix {conf.mpi_path} "
        prefix_cmd += (
            f" -x {conf.mpi_env}"
